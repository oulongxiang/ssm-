...
<div>
	<form action="${pageContext.request.contextPath}/UserServlet?act=update" method="post" style="width:100%;text-align:center;">
		<input name="userid" type="hidden" value='<%=user.getUser_id() %>'>
        <table class="wzc-table">
			<tr>
                <th>用户名</th>
                <td><input name="username" type="text" readonly value='<%=user.getUsername()==null?"":user.getUsername()%>'></td>
                <th>昵称</th>
                <td><input name="nickname" type="text" value='<%=user.getPassword()==null?"":user.getPassword()%>'></td>
            </tr>
			<tr>
                <th>密码</th>
                <td><input name="password" type="text" value='<%=user.getRolename()==null?"":user.getPassword()%>'></td>
                <th>性别</th>
                <td><input name="gender" type="text" value='<%=user.getSex()==null?"":user.getSex()%>'></td>
            </tr>
			<tr>
                <th>手机号</th>
                <td><input name="phone" type="text" value='<%=user.getDesc()==null?"":user.getPhone()%>'></td>
                <th>邮箱</th>
